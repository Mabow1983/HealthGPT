...
class HealthDataFetcher {
    ...
    func requestAuthorization(completion: @escaping (Bool) -> Void) {
        ...
        guard HKHealthStore.isHealthDataAvailable(),
              ...
              let glucose = HKObjectType.quantityType(forIdentifier: .bloodGlucose) else {
            completion(false)
            return
        }
...
if let samples = samples as? [HKCategorySample] {				
    ...
    func fetchLastTwoWeeksInsulin(completion: @escaping ([Double]) -> Void) {
    fetchLastTwoWeeksQuantityData(
        for: .insulinDelivery,
    ...
    func fetchLastTwoWeeksGlucose(completion: @escaping ([Double]) -> Void) {
        fetchLastTwoWeeksQuantityData(
            for: .bloodGlucose,
